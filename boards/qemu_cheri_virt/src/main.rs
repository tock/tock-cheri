...
use capsules_core::{
    ...
    virtualizers::{
        virtual_alarm::VirtualMuxAlarm, virtual_uart_zero::mux3,
...
static mut BOARD: Platform = {
    ...
    kernel::construct_grants!(board_kernel, proto, counter, &memory_allocation_cap, {
        alarm_grant : capsules_core::alarm::DRIVER_NUM,
        console_grant : capsules_core::console::DRIVER_NUM,
        lldb_grant : capsules_core::low_level_debug::DRIVER_NUM,
        cheri_grant : capsules_core::driver::NUM::MMU as usize,
    });
...
fn main() {
    ...
    let (flash, mem) = unsafe { kernel::process_loading::get_mems() };
...
impl kernel::platform::SyscallDriverLookup for Components {
    ...
    {
        use capsules_core::low_level_debug::LowLevelDebugZero;
        const MMU_NUM: usize = capsules_core::driver::NUM::MMU as usize;
        f(match driver_num {
            capsules_core::console::DRIVER_NUM => {
                Some(Console::get_syscall_driver(mux3::a(&self.uart_and_console)))
            }
            capsules_core::alarm::DRIVER_NUM => Some(&self.alarm_capsule),
            capsules_core::low_level_debug::DRIVER_NUM => Some(
                LowLevelDebugZero::get_syscall_driver(mux3::b(&self.uart_and_console)),
            ),
