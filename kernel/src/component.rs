...
/// initialization for the Tock kernel in a factory method, which reduces
/// repeated code and simplifies the boot sequence.
///
/// The [`Component`] trait encapsulates all of the initialization and
/// configuration of a kernel extension inside the [`Component::finalize()`]
/// function call. The [`Component::Output`] type defines what type this
/// component generates. Note that instantiating a component does not
/// instantiate the underlying [`Component::Output`] type; instead, the memory
/// is statically allocated and provided as an argument to the
/// [`Component::finalize()`] method, which correctly initializes the memory to
/// instantiate the [`Component::Output`] object. If instantiating and
/// initializing the [`Component::Output`] type requires parameters, these
/// should be passed in the component's `new()` function.
///
...
pub trait Component {
    ...
    /// that [`crate::static_buf!()`] would normally setup, but generic
    ...
    /// [`Component::StaticInput`] type makes this possible.
    type StaticInput;
    ...
    /// convention:
    ///
    /// - All statically allocated memory MUST be passed to
    ///   [`Component::finalize()`] via the `static_memory` argument. The
    ///   [`Component::finalize()`] method **MUST NOT** use
    ///   [`crate::static_init!()`] or [`crate::static_buf!()`] directly. This
    ///   restriction ensures that memory is not aliased if the component is
    ///   used multiple times.
