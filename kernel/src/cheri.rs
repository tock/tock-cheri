...
pub trait CPtrOps {
    ...
    fn set_addr_from_ddc_restricted(&mut self, addr: usize, base: usize, len: usize) {
        ...
        self.set_bounds(len);
        self.set_addr(addr);
        self.and_perms(cheri_perms::DEFAULT_RW);
    }
...
macro_rules! csr_op {
    {$REG : tt <- $SRC : tt} =>
        {concat!(csr_ptr!(), "w", " ", $REG, ptrreg!(), ", ", ptrreg!($SRC))};
    {$REG : tt -> $DST : tt} =>
        {concat!(csr_ptr!(), "r", " ", ptrreg!($DST), ", ", $REG, ptrreg!())};
    {$DST : tt <- $REG : tt <- $SRC : tt} =>
        {concat!(csr_ptr!(), "rw", " ", ptrreg!($DST), ", ", $REG, ptrreg!(), ", ", ptrreg!($SRC))};
}
...
/// such that they can be represented. Both length, bottom, and top must be rounded using the
/// mask.
