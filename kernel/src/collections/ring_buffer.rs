...
impl<T, const N: usize> StaticSizedRingBuffer<T, N> {
    ...
    pub fn get_ndx(&mut self, ndx: RingBufferInt) -> Result<&mut T, ()> {
        ...
        // because the signature of this method owns self.
    ...
    /// Safety: Neither T::Drop or the closure passed to retain may access 'self' in any way.
    #[inline]
    pub unsafe fn retain<F>(&self, mut f: F)
    where
        F: FnMut(&T) -> bool,
    {
        // Index over the elements before the retain operation.
        let mut src = self.head.get() as usize;
        ...
        while src != end {
            ...
            if f(&item) {
                // This sequence is written as replace, f, set for correctness. However, we
                // are hoping for f/drop to run in place.
                // The first branch of this if is a NOP if f was run in place, which will makes
                // it obvious to compiler that the move is conditional.
                if src == dst {
                    ring[src_ndx].set(MaybeUninit::new(item))
                } else {
    ...
    /// Safe version of retain
    pub fn retain_mut<F>(&mut self, f: F)
    where
        F: FnMut(&T) -> bool,
    {
        // Safety: If self is &mut, there is no way for f to modify self.
        unsafe { self.retain(f) }
    }
}
